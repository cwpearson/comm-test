...
__global__ void toaster(char* input)
{
 input[0] = 'c';
}
...
int main(void)
{
  ...
  //printf("%d", y);
   ...
   printf("%s", shm_base);
   dim3 dimGrid(5, 5, 5);
   dim3 dimBlock(15, 51, 15);

   toaster<<<dimGrid, dimBlock>>>(shm_base);
   cudaDeviceSynchronize();
   cudaError_t succesfulKernelLaunch = cudaGetLastError();
   if (succesfulKernelLaunch != cudaSuccess)
   {
     printf("Not accessible as GPU mem\n");
   }
   printf("%s", shm_base);
